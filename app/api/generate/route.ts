...
⚠️ "쉽게 비유하자면"을 글 전체에서 2회 이상 반복하지 마세요! 패턴을 돌려서 사용!
...
- Q&A 2~3개 자연스럽게 삽입 ("많이들 궁금해하시는 부분인데요.")
- 치료 기간/비용 범위 등 실용 정보를 문장 속에 녹여서 전달
- "~하죠", "~시죠" 활용하여 독자 참여 유도
...
### 2. FAQ Q&A 블록 (스마트블록 + AI 추출용)
- 본론에 Q&A **최소 2개** 배치 (검색 의도 반영)
- Q. [자연어 질문 형태, 메인키워드 포함]
- A. [2~3문장 명확한 단정형 답변, 수치 포함]
- AI가 Q&A를 우선 추출하므로 답변을 명확하고 **독립적**으로 작성
...
- **3막 해결**: 치료법을 이야기식으로 + Q&A 삽입 → "많이들 궁금해하시는 부분인데요."
...
- [ ] Q&A 블록이 2개 이상 있고, 답변이 독립적이고 명확한가?
...
- 🚨 "무엇보다", "가장", "특히" 등 강조 부사는 글 전체에서 최대 2회까지만 사용하세요! (중복 시 즉시 삭제됨)
