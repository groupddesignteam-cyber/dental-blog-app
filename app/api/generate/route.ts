...
- 치료 후 주의사항 (전문가 관점)
- 가능한 합병증과 대처법
- 추가 Q&A 1~2개 (답변을 3~4문장으로 충분히 설명, 짧은 답변으로 끝내지 않기)
...
## 용어 치환 규칙 (SEO + 독자 이해 최적화)
${Object.entries(TERM_REPLACEMENTS).map(([k, v]) => `- ${k} → ${v}`).join('\n')}

### 🚨 필수 대치어 (절대 단독 사용 금지!)
아래 용어는 **원어를 단독으로 쓰지 말고** 반드시 대치어를 사용하세요:
- 크라운 → **지르코니아 보철**, 심미 수복물, 보강관 (❌ "크라운" 단독 사용 절대 금지!)
- 치아 균열 → 크랙, 자연치 크랙, 치아 금 (전문 용어와 일상 용어를 병기)
- 충치 → 치아 우식 (첫 등장 시 "치아 우식(충치)" 형태로 병기)
- 임플란트 → 인공치아 식립 (문맥에 따라 "임플란트"도 허용하되 단독 반복 자제)

### 📝 한국어 문장 자연스러움 규칙 (매우 중요!)
⚠️ 형태소 치환이나 동의어 교체 시 **조사와 어미가 어긋나지 않도록** 주의하세요!
용어를 바꿔 넣을 때 문장이 번역투(직역)처럼 어색해지면 안 됩니다.

**필수 준수:**
- 연결 어미를 자연스럽게: ~하며, ~하기 때문에, ~을 통해, ~으로 인해, ~하게 됩니다
- 조사 정확성: 이/가, 은/는, 을/를이 문맥에 맞아야 함
- 직역투 금지: "치료가 시행이 됩니다" → "치료를 진행하게 됩니다"
- 주어-서술어 호응: 긴 문장에서도 주어와 서술어가 자연스럽게 연결

❌ 나쁜 예 (형태소 어긋남):
"지르코니아 보철를 장착을 합니다" → 조사 어긋남
"치아 우식이 진행이 되어" → 이중 조사, 번역투

✅ 좋은 예 (자연스러운 한국어):
"지르코니아 보철을 장착하게 됩니다"
"치아 우식이 진행되어"
"이러한 증상이 나타나는 경우, 정밀 검사를 통해 정확한 진단이 이루어집니다."
...
**Q&A 블록 → 본론 통합** (네이버 스마트블록 + 글 퀄리티 향상)

⚠️⚠️ 핵심 원칙: Q&A를 단순 문답으로 끝내지 말 것! ⚠️⚠️
Q&A에서 도출된 핵심 답변과 전문적인 설명을 본론 각 단락에 **서술형으로 자연스럽게 녹여내어** 전체 글자수를 확보하라.
독자가 궁금해할 법한 내용을 본문에서 부연 설명하며 풍성하게 작성하라.

**Q&A → 본론 통합 패턴:**
Q. [검색 의도 반영 질문 - 메인키워드 포함]?
A. [핵심 답변 2~3문장]
→ 이 답변을 바로 아래 섹션 1에서 **서술형으로 확장**하여 설명

❌ 나쁜 예 (Q&A가 독립적으로 끝남):
"Q. 치경부 마모증이 뭔가요? A. 치아와 잇몸 경계가 패이는 증상입니다. ---"
→ 답변 후 곧바로 다른 주제로 전환 (분량 부족, 깊이 없음)

✅ 좋은 예 (Q&A가 본론으로 확장):
"Q. 치경부 마모증이 뭔가요? A. 치아와 잇몸이 만나는 경계 부위가 패이면서 상아질이 드러나는 상태입니다."
→ 바로 이어서: "쉽게 비유하자면, 치아는 여러 겹의 보호막을 입고 있습니다. 가장 바깥쪽은 '법랑질'이라는 단단한 갑옷이고..." (본론 섹션 1로 자연스럽게 확장)

**본론 섹션 1**: ✅ [원인/증상 설명] (600자 이상)
- Q&A 답변에서 출발하여 서술형으로 풍성하게 확장
- 왜 이런 문제가 생기는지 (상세하게 3~4문단, 서술형)
...
**본론 섹션 2**: 🔹 [치료 방법/과정] (600자 이상)
- 치료 과정을 서술형으로 자연스럽게 풀어서 설명 (단계별, 수치 포함)
- CC에 언급된 치료법을 일반화하여 설명
- 독자가 궁금해할 "왜 이 치료를 하는지", "어떤 과정인지"를 부연
- "치과" 2회 분산 포함

**본론 섹션 3**: 🔵 [주의사항/관리] (400자 이상)
- 치료 후 주의사항 (불릿 리스트 형태)
- 관리 방법 및 팁
- 추가 Q&A 1~2개 → 답변을 3~4문장으로 충분히 풀어서 설명 (단문 답변 금지!)
...
1. **Q&A → 본론 서술형 통합 (핵심!)**
   - Q&A 핵심 답변 2~3문장 (단정적, 명확)
   - 답변을 본론에서 **서술형으로 풍성하게 확장** (단순 나열 X, 부연 설명 O)
   - 독자가 궁금해할 법한 내용을 본문에서 자연스럽게 다루기
   - "~입니다", "~됩니다" 형태로 단정적 답변
...
// 유효하지 않은 원장님 이름 필터링
function sanitizeDoctorName(name: string): string {
  const sanitized = sanitizeInput(name)
  const invalidNames = ['링크 없음', '링크없음', '없음', 'N/A', 'n/a', '-', '']
  if (invalidNames.some(invalid => sanitized.trim() === invalid)) return ''
  return sanitized
}
...
function sanitizeFormData(data: GenerateFormData): GenerateFormData {
  return {
    ...
    doctorName: sanitizeDoctorName(data.doctorName),
...
- 치과명: ${data.clinicName}
- 지역: ${data.region}
- 원장님 이름: ${data.doctorName || '(미입력 — 서문/마무리 인사에서 이름 생략)'}
- 주제/치료: ${data.topic}
- 치료 상황 참고 (⚠️ 직접 언급 금지!): ${data.patientInfo}
- 치료 방법 참고: ${data.treatment}
${data.photoDescription ? `- 이미지 참고 정보: ${data.photoDescription}` : ''}

## 🚫 환자 정보 직접 언급 금지 (의료법 위반!!)
위 "치료 상황 참고"와 "치료 방법 참고"는 글 작성 시 **참고용**입니다.
- ❌ "환자분", "이번 환자분의 경우", "00대 여성 환자분" 등 직접 언급 금지
- ❌ 환자 연령, 성별, 구체적 상황 언급 금지
- ✅ "이런 증상이 있으신 분들", "이런 경우" 등 일반화해서 작성

## 🚫 치과명 본문 사용 금지 (의료법 위반!!)
치과명 "${data.clinicName}"은 아래 2곳에서**만** 사용:
1. 서문 인사: "안녕하세요, ${data.region} ${data.clinicName} ${data.doctorName || '원장'}입니다."
2. 마무리 인사: "${data.region} ${data.clinicName} ${data.doctorName || '원장'}이었습니다."
- ❌ 본문에서 "${data.clinicName}에서는~", "저희 치과에서~" 절대 금지
- ✅ "가까운 치과에서 상담을 받아보시길 권장합니다"
...
### SEO 키워드 조합
- "${data.region} ${data.clinicName}" → 서문 인사 1회 + 마무리 인사 1회 = 총 2회만! 본문에서 치과명 사용 금지!
...
### 🎯🎯🎯 메인키워드 "${mainKeyword}" 배치 규칙 (최우선!!) 🎯🎯🎯

**메인키워드 "${mainKeyword}" 총 7회, 반드시 분산 배치:**
- 제목: 1회 (앞쪽에 배치)
- 서론: 1회 (인사말 또는 초반 200자 이내)
...
**✅ 메인키워드 활용 시 허용 표현:**
- "${mainKeyword}에서 준비한~", "${mainKeyword}에서는 ~을 권장합니다"
- "~라고 생각합니다", "~로 평가합니다", "~로 판단합니다"
- "~로 보입니다", "~일 수 있습니다", "~을 추천합니다"

**❌ 메인키워드 활용 시 절대 금지 표현:**
- "${mainKeyword}에서 ~를 해결했습니다" (확정 짓는 표현 금지!)
- "저희 ${mainKeyword}에서 ~를 시행합니다" (내원유도 금지!)

**나머지 키워드: 각 5~6회 이하 (한 문단에 몰리지 않도록 분산!)**

### 문장/문단 규칙 (모바일 최적화 + 자연스러운 한국어!)
...
9. **🚨 문장 자연스러움 (최우선!)**: 용어 치환/동의어 교체 시 조사(이/가, 은/는, 을/를)가 어긋나거나 번역투가 되면 안 됨! 매끄러운 연결 어미(~하며, ~하기 때문에, ~을 통해) 사용. 형태소 규칙에 매몰되어 문맥이 깨지는 것보다 **자연스러운 한국어가 우선**!

### 키워드 배치 규칙 (분산 필수!)
1. 메인키워드 "${mainKeyword}": **7회** (제목 1 / 서론 1 / 본론 4 / 결론 1) ← 최우선!
2. 치료 키워드 "${data.topic}": **3~5회** (서론 1~2회 / 본론 2~3회 / 결론 1회)
3. 서브 키워드: 각 2~3회, 본론에 분산 배치
4. ❌ 한 문단에 같은 키워드 2회 이상 사용 금지!
5. ✅ 메인키워드 "${mainKeyword}" 형태 그대로 사용
...
export async function POST(request: NextRequest) {
  try {
    ...
    const systemPrompt = buildSystemPrompt(primaryTopic, clinicPersona, data.writingMode)
    // 배치 다양성 지시 생성
