...
**목표**: CC(치료 상황)와 이미지 파일명을 기반으로 한 전문적 임상 글

**⚠️⚠️ 정보성(백과사전식) 글과 완전히 다릅니다! ⚠️⚠️**
- ❌ 정보성: "임플란트란 무엇일까요?" → 사전적 설명 중심
- ✅ 임상: "치조골 흡수가 관찰되는 경우, 골이식 후 임플란트 식립이 고려됩니다." → 소견 기반
...
- CC에 "어금니 통증" → 서론에서 "어금니 부위에 통증을 호소하시는 경우가 많습니다" 형태로 활용
- CC에 "뼈가 부족" → 본론에서 "치조골이 부족한 경우 골이식이 선행됩니다" 형태로 활용
- CC에 구체적 치료법 → 본론에서 해당 치료의 단계별 과정을 상세 설명
- ❌ 단, "이번 환자분" 등 특정 환자 직접 언급은 절대 금지!
- ✅ "이러한 소견이 보이는 경우", "이런 상황에서는" 형태로 일반화

**글 전개 흐름 (필수!)**:
소견 관찰 → 진단 → 치료 계획 → 치료 과정 → 예후/관리

**서론 (500자) - 임상 소견 기반 도입:**
❌ "임플란트란 무엇일까요?" (정보성 도입 - 금지!)
❌ "혹시 이런 경험 있으신가요?" (정보성 도입 - 금지!)
✅ 인사 → 시의성 훅 → "이런 증상을 느끼시는 분들이 계십니다" (CC 연결) → 소견 기반 도입
✅ "방사선 사진상 ~가 관찰되는 경우가 있습니다." (임상 도입)
✅ "임상 사진상 ~부위에 ~이 관찰되는 경우, 적절한 치료가 필요합니다." (임상 도입)

**본론 (1,500자) - 소견 + 치료 과정 상세:**
- "~가 관찰됩니다" (객관적 기술, 최소 3회)
- "이는 ~를 시사하는 소견입니다" (진단적 해석)
- "이러한 경우 ~가 고려됩니다" (치료 방향)
- 치료 단계별 상세 과정 (수치/기간 포함)
- 주의사항과 합병증 가능성
- 추가 Q&A 1~2개
...
**목표**: 일반인 눈높이의 깊이 있는 치과 정보 글 (인터넷 검색으로 나오는 뻔한 내용 금지!)
...
- ❌ "임플란트는 인공 치아를 심는 시술입니다." (사전적 설명만)
- ✅ "치수 조직까지 감염이 진행되면 자발통이 나타납니다. 이 단계에서는 근관치료가 필요하며, 보통 2~3회 내원하여 치료를 진행합니다." (구체적 + 전문적)
- ✅ "임플란트 식립 시 치조골 높이가 최소 8mm 이상이어야 합니다. 부족한 경우 골이식이 선행되며, 골이식 후 약 4~6개월의 치유 기간이 필요합니다." (수치 + 단계)

**필수 포함 내용 (깊이 있게! 각 항목 최소 3~4문단!):**
1. 증상의 원인 메커니즘 (왜 발생하는지 과학적 설명 + 비유)
2. 단계별 진행 과정 (방치하면 어떻게 되는지 - 구체적 수치)
3. 치료 방법별 비교 (장단점 + 비용 범위 + 기간)
4. 구체적 수치/기간 명시 ("보통 3~6개월", "약 95~98% 성공률")
5. 자주 묻는 질문 2~3개 (Q&A 형태)
6. 비유 표현 필수 (전문 개념마다 일상 비유 1개 이상)
7. 일상에서 실천 가능한 관리 팁 (구체적으로)

**서론 500자 / 본론 1,500자 / 결론 500자를 반드시 채우세요!**

**예시 문장**:
- "많은 분들이 궁금해하시는 내용입니다."
- "쉽게 비유하자면, 마치 ~와 유사합니다."
- "그렇다면 왜 이런 현상이 발생하는 걸까요?"
- "수치로 보면, 약 ~% 정도에서 이러한 양상이 나타납니다."
`
  }
