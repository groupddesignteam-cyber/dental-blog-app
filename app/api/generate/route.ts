...
⚠️ 배치 규칙:
- **제목·서론·마무리**: 반드시 A+B 합쳐서 (예: "${mainKeyword}")
- **본론**: 합쳐서 또는 따로 사용 가능!
  예) "${data.region}에서는..." (A만) / "${mainKeyword.replace(data.region, '').trim() || '치과'}를..." (B만) / "${mainKeyword}" (합쳐서)
- ❌ 한 문단에 동일 형태소 2회 이상 금지!
- ✅ 각 형태소 7회 맞추기 위해 본론에서 자유롭게 분산
...
- 형태소B에 포함되어 있으므로 별도 단독 반복은 2~3회 이내만 허용`}
