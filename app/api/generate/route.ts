...
5. **동의어/관련어 활용**: 같은 단어가 글 전체에서 7회 이상 반복되면 반드시 동의어로 교체!
...
### 치료 키워드: "${data.topic}"
${mainKeyword.includes(data.topic) ? `⚠️ 메인키워드 "${mainKeyword}" 안에 이미 "${data.topic}"이 포함되어 있습니다!
- 메인키워드 7회 배치 시 자동으로 포함되므로, "${data.topic}" 별도 단독 반복은 전체 2~3회 이내만 허용!
- ❌ "${data.topic}"을 단독으로 추가 배치하지 마세요!` : `- 독립적으로 자연스럽게 3~5회 배치 (초반200자, 중간, 마지막200자 포함)`}
