...
export const SYNONYM_DICTIONARY: Record<string, string[]> = {
  // 치아 관련
  '치아': ['자연치'],  // 영구치(permanent teeth)는 별개 개념 → 제외
  ...
  // '임플란트': ['인공치아', '임플란트 시술'],
  '발치': ['치아 제거', '치아 적출'],  // 발거(extraction)는 "발치와(extraction socket)"와 혼동 → 제외
  '수복': ['복원', '회복', '재건'],
  '식립': ['매식', '배치'],  // 설치(installation)는 비의학적 → 제외, 이식(transplant)은 별개 → 제외
  '골이식': ['뼈이식', '골보충', '골대체'],
  '상악동거상술': ['사이너스 리프트', '상악동 수술', '상악동 거상'],
  '보철': ['수복물', '보철 수복'],  // 지르코니아는 재료명이지 보철 동의어 아님 → 제외
  '스케일링': ['치석 제거', '치면 세마', '잇몸 청소'],

  // 상태/증상 관련
  '통증': ['아픔', '동통', '통증성 반응'],  // 압통(tenderness)·불편감(discomfort)은 의학적으로 별개 개념 → 제외
  '염증': ['발적', '염증 반응', '염증성 변화'],  // 감염(infection)은 염증(inflammation)과 별개 → 제외
  '증상': ['양상', '소견', '징후'],
  '손상': ['파손', '결손', '파절', '마모'],
  '흡수': ['소실', '감소', '위축'],
  ...
  // 치료 과정 관련
  // '진행': 삭제 — "감염이 진행된"→"감염이 시행된" 의미 파괴. 임상 표준 용어.
  '적용': ['사용', '활용'],  // '배치'는 식립 동의어와 중복 → 제외
  '완료': ['마무리', '종료'],

  // 일반 서술
  '경우': ['상황', '사례'],  // '케이스'는 임상 글 톤에 부적합 → 제외
  // '부위': 삭제 — 해부학 표준 용어. "43번 부분는" 등 부자연스러운 교체 방지.
  '관리': ['유지', '사후 관리'],  // '케어'는 외래어 남용 → 제외
  '중요': ['핵심적', '필수적', '주요한'],

  '확인': ['파악', '점검'],  // '살펴보기'는 구어체 → 제외
  // '상태': 삭제 — "골유착 수행 현황" 등 행정 문서 톤 방지. 임상 표준 용어.

  // 추가 동의어 (v3.11.0)
  '조직': ['연조직', '조직층'],  // 세포(cell)≠조직(tissue) 의학적 오류 → 제외
  '방법': ['방식', '기법', '접근법'],
  '부족': ['결핍', '미달', '미흡'],
  '증가': ['상승', '증대', '확대'],
  '예방': ['방지', '선제적 관리', '사전 관리'],
